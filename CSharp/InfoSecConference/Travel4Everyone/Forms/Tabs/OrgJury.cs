...
{
    public partial class OrgJury : UserControl
    {
        ...
        {
            byte[] imageData = DBNull.Value.Equals(record[9]) ? null : (byte[])record[9];
            Image image = ByteArrayToImage(imageData);

            _ = dgw.Rows.Add(record.GetInt32(0), record.GetString(1), record.GetString(2), record.GetString(3), record.GetDateTime(4), record.GetString(5), record.GetString(6), record.GetString(7), record.GetString(8), image);
        ...
        {
            dgw.Rows.Clear();
            string querystring = "SELECT ID, FullName, Gender, Email, DateOfBirth, CountryCode, PhoneNumber, Direction, Password, Photo FROM Jury";
        ...
        {
            string deleteQuery = "DELETE FROM Jury WHERE ID = @ID";
