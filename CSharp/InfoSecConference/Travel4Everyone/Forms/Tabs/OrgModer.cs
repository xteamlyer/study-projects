...
{
    public partial class OrgModer : UserControl
    {
        ...
        {
            byte[] imageData = DBNull.Value.Equals(record[10]) ? null : (byte[])record[10];
            Image image = ByteArrayToImage(imageData);

            _ = dgw.Rows.Add(record.GetInt32(0), record.GetString(1), record.GetString(2), record.GetString(3), record.GetDateTime(4), record.GetString(5), record.GetString(6), record.GetString(7), record.GetString(8), record.GetString(9), image);
        ...
        {
            dgw.Rows.Clear();
            string querystring = "SELECT ID, FullName, Gender, Email, DateOfBirth, CountryCode, PhoneNumber, Direction, EventName, Password, Photo FROM Moderators";
        ...
        {
            string deleteQuery = "DELETE FROM Moderators WHERE ID = @ID";
        ...
        {
            AddModer addModer = new AddModer();
            addModer.Show();
        }
